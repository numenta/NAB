...
class GrokDetector(AnomalyDetector):

  def __init__(self, *args, **kwargs):

    # Init the super class
    super(GrokDetector, self).__init__(*args, **kwargs)
    ...
    grokLearningPeriod = math.floor(self.probationaryPeriod / 2.0)
  ...
  def handleRecord(self, inputData):
    ...
    # Send it to Grok and get back the results
...
class AnomalyLikelihood(object):
  ...
  def __init__(self, probationaryPeriod = 600, grokLearningPeriod = 300):
    """
    probationaryPeriod - no anomaly scores are reported for this many
    iterations.  This should be grokLearningPeriod + some number of records
    for getting a decent likelihood estimation.

    grokLearningPeriod - the number of iterations required for Grok to
    ...
    self._grokLearningPeriod  = grokLearningPeriod
      ...
      if self._distribution is None or (self._iteration % 100 == 0): 
        _, _, self._distribution = (
          anomaly_likelihood.estimateAnomalyLikelihoods(
            self._historicalScores,
            skipRecords = self._grokLearningPeriod)
          )
